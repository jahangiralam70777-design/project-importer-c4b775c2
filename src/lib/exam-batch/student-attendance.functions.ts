...
function emptyDecision(reason: ExamBatchAccessDecision["reason"]): ExamBatchAccessDecision {
  return {
    allowed: reason === "ok",
    reason,
    banned: false,
...
export const getExamBatchAccessState = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }): Promise<ExamBatchAccessDecision> => {
    const visible = await readModuleVisible(context.supabase);
    if (!visible) return { ...emptyDecision("hidden"), allowed: false };
    ...
    if (bannedRows.length === 0) return emptyDecision("ok");
    ...
    const bans = bannedRows.map((r) => buildBanInfo(r, settings, sessions, subjects));
