...
const STATE_COLUMNS =
  "user_id,session_id,subject_id,consecutive_missed_count,last_missed_exam_id,last_missed_at,last_attended_exam_id,last_attended_at,banned,banned_at,banned_reason,banned_by,auto_banned,updated_at";
...
async function enrichStates(
  ...
  const [profiles, sessions, subjects, enrollments] = await Promise.all([
    userIds.length
      ? supabase
          .from("profiles")
    ...
    userIds.length && sessionIds.length
      ? supabase
          ...
          .select("user_id,session_id,student_id")
          .in("user_id", userIds)
  ...
  for (const s of sessions as any[]) sessMap.set(s.id, s.title ?? null);
  ...
  for (const e of enrollments as any[]) {
    if (e.student_id != null) sidMap.set(`${e.user_id}::${e.session_id}`, Number(e.student_id));
  }
  ...
  return rows.map((r) => ({
    ...
    sessionTitle: sessMap.get(r.sessionId) ?? null,
    subjectName: subjMap.get(r.subjectId) ?? null,
    studentId: sidMap.get(`${r.userId}::${r.sessionId}`) ?? null,
  }));
}
...
export const adminListExamBatchAttendanceStates = createServerFn({ method: "POST" })
  ...
  .handler(
    async ({
      ...
      if (error) mapSupabaseError(error, "adminListExamBatchAttendanceStates");
      let states = (rows ?? []).map(mapState);

      // Search is applied post-enrichment against student name / email.
      let enriched = await enrichStates(context.supabase, states);
      if (data.search) {
      ...
      return { rows: enriched, total: Number(count ?? enriched.length) };
    },
  );
...
async function performManualBan(
  supabase: any,
  actorId: string,
  scope: { userId: string; sessionId: string; subjectId: string; reason: string },
): Promise<AttendanceState> {
