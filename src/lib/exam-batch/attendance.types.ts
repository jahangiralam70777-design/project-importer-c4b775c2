...
export const attendanceSettingsSchema = z.object({
  ...
  // e.g. limit=4, nearBanOffset=1 → warn at count>=3.
...
export interface ExamBatchAccessDecision {
  allowed: boolean;
  // `banned` = at least one (session,subject) ban is active on this account.
  // `hidden` = whole module hidden by admin.
  // `guest`  = not authenticated.
  reason: "ok" | "banned" | "hidden" | "guest";
  banned: boolean;
  activeBanCount: number;
  // Primary ban surfaced to the UI (most recent). Null when not banned.
  ban: ExamBatchBanInfo | null;
  // All active bans for the current student, so the UI can list every
  // (session,subject) the student is blocked from.
