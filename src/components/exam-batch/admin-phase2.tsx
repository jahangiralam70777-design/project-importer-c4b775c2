  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
        <SectionCard title="Frozen exams" description="Last 45 days" className="lg:col-span-1">
          <select
            value={sessionFilter}
            onChange={(e) => setSessionFilter(e.target.value)}
            className="mb-3 h-10 w-full rounded-xl border border-input bg-background/60 px-3 text-sm"
          >
            <option value="all">All sessions</option>
            {sessions.map((s) => (
              <option key={s.id} value={s.id}>
